...
void main()
{
    ...
    plist[0] = (int *)malloc(sizeof(int)); // 정수형 포인터의 0번째 인덱스 위치에 동적할당
    // (첫번째 인덱스는 해당 동적할당된 힙의 주소를 저장할 것임)

    list[0] = 1; // 정수형 배열의 0번째 인덱스 위치에 1 대입
    list[1] = 100; // 정수형 배열의 1번째 인덱스 위치에 100 대입

    *plist[0] = 200; // 정수형 포인터 배열의 0번째 인덱스가 가리키고 있는 주소(힙의 어딘가의 주소)에 200 대입
    ...
    printf("*(list+1) = %d\n", *(list + 1)); // list+1의 주소를 역참조하여, 100 출력될 것임 (list[1]과 동일)
