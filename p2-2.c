...
int main()
{
    ...
    printf("&one[0] = %p\n", &one[0]); // one[0]의 주솟값 출력. 위와 동일. 다 같은 주소를 반환함. 단, one과 &one[0]는 정말 표현법만 다르고 동일하다고 볼 수 있지만, one과 &one은 조금 다름.
    // &one은 배열 그 자체를 의미하도록 되있음. &one을 통해 얻는 자료형이 int(*)[5]라고 생각하면 됨. 그렇기 때문에, 배열 포인터에 대입해줄때 해당 방식으로 적게 되있음. 
    // 단순한 일반 포인터에 배열의 주소를 넣을 때는, one으로 적어야 함. one을 통해 얻는 자료형은 int *이라고 생각하면 됨 (&one[0]과 동일).
    printf("\n");
    printf("------------------------------------\n");
    ...
    // 만약 여기에 one 대신 &one으로 적어도, 실행이 정상적으로 됨.(print_one 함수 내에서는 어찌되었든 받은 주솟값을 바탕으로, 본래의 ptr의 자료형인 int*에 맞추어 동작하기 때문)
...
{ // 포인터를 이용해 일차원 배열을 출력하는 함수
    ...
    {
        printf("%p \t %5d\n", ptr + i, *(ptr + i)); // ptr + i와, 그 주소를 역참조한 것을 출력.
        // 아무것도 더하지 않은 상태부터 1을 더해줄 때마다 주소가 4씩 뒤로 가면서, 차례대로 해당하는 주소와, 해당 인덱스 위치의 원소가 출력될 것임. (row가 5라면, 0 인덱스 ~ 4 인덱스까지 출력될 것임.) 
        // 즉, 4씩 증가되는 주소와, 0, 1, 2, 3, 4 가 출력될 것임.
        // i를 더해주면 4i씩 뒤로 가는 이유는, ptr이 int형 포인터 변수이기 때문임.
    }
    printf("\n");
}
